    ...
    public List<List<String>> init(Map<String, List<Integer>> studentTimeMap) {
        ...
        for (Map.Entry<String, List<Integer>> entry : studentTimeMap.entrySet()) { // entrySet(): Map의 전체 (key, value) 쌍 꺼내는 메소드
            ...
            for (int time : entry.getValue()) {
                // computeIfAbsent
                // key가 있으면 -> 해당 value 리턴
                // key가 없으면 -> 빈 value 만들어서 생성해서 Map timeToStudents에 추가 후 반환 . 여기서는 new ArrayList<>() 생성
                // add로 방금 얻은 리스트에 student 추가
                timeToStudents.computeIfAbsent(time, k -> new ArrayList<>()).add(student);
            }
        }

        // 각 시간아이디별로 신청자리스트에서 3~5명 가능조합 모두 만들어서 teamCandidates에 추가
